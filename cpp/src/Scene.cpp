...
// Constructor
//-----------------------------------------------------------------------------
		Scene::Scene(uint8 const _sceneId) :
				m_sceneId(_sceneId), m_label("")
		{
...
//-----------------------------------------------------------------------------
		void Scene::WriteXML(string const& _name)
		{
			char str[16];
...
//-----------------------------------------------------------------------------
		Scene* Scene::Get(uint8 const _sceneId)
		{
...
//-----------------------------------------------------------------------------
		uint8 Scene::GetAllScenes(uint8** _sceneIds)
		{
			if (s_sceneCnt > 0)
			{
...
//-----------------------------------------------------------------------------
		bool Scene::AddValue(ValueID const& _valueId, string const& _value)
		{
...
//-----------------------------------------------------------------------------
		bool Scene::RemoveValue(ValueID const& _valueId)
		{
...
//-----------------------------------------------------------------------------
		void Scene::RemoveValues(uint32 const _homeId, uint8 const _nodeId)
		{
			for (int i = 1; i < 256; i++)
			{
...
//-----------------------------------------------------------------------------
		int Scene::GetValues(vector<ValueID>* o_value)
		{
...
//-----------------------------------------------------------------------------
		bool Scene::GetValue(ValueID const& _valueId, string* o_value)
		{
			for (vector<SceneStorage*>::iterator it = m_values.begin(); it != m_values.end(); ++it)
			{
				if ((*it)->m_id == _valueId)
...
//-----------------------------------------------------------------------------
		bool Scene::SetValue(ValueID const& _valueId, string const& _value)
		{
			for (vector<SceneStorage*>::iterator it = m_values.begin(); it != m_values.end(); ++it)
			{
				if ((*it)->m_id == _valueId)
