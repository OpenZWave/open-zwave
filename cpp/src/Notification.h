...
namespace OpenZWave
{
	...
	class OPENZWAVE_EXPORT Notification
	{
			...
			friend class Internal::CC::SwitchMultilevel;
			friend class Internal::CC::Color;
		...
		public:
			...
			enum NotificationType
			{
				...
				Type_LevelChangeStart, /**< Start level changes (multilevel switch, color) */
				Type_LevelChangeStop /**< Stop level changes (multilevel switch, color) */
			};
			...
			/**
			 * Types of things that support start/stop of level changes.
			 */
			enum LevelChangeType
			{
				LevelChange_Switch,  /**< Multilevel Switch */
				LevelChange_Color    /**< Color Switch start/stop level change */
			};

			/**
			 * Interpretation of the up/down bits in a level change command.
			 * For color change, this is just up or down.  For a multilevel switch with
			 * a secondary control, could ignore one or the other.
			 */
			enum LevelChangeDirection
			{
				LevelChangeDirection_Up,  /**< When the change is started, increment. */
				LevelChangeDirection_Down, /**< When the change is started, decrement. */
				LevelChangeDirection_None /**< Do not increment or decrement.  Used for multilevel switch with 2 switches. */
			};

			/**
			 * Used for storing the parameters for a multilevel switch or color change start command.
			 * Only a multilevel switch has the secondary parameters.
			 */
			struct LevelChangeParameters
			{
				LevelChangeParameters():
					m_type( LevelChange_Switch ),
					m_primaryDirection(LevelChangeDirection_None),
					m_secondaryDirection(LevelChangeDirection_None),
					m_ignoreStartLevel(0),
					m_primaryStartLevel(0),
					m_durationSeconds(0),
					m_secondaryStepSize(0),
					m_colorTarget(""){}

				LevelChangeType m_type;
				LevelChangeDirection m_primaryDirection;
				LevelChangeDirection m_secondaryDirection;
				uint8 m_ignoreStartLevel;
				uint8 m_primaryStartLevel;
				uint16 m_durationSeconds;
				uint8 m_secondaryStepSize;
			...
			/**
			 * Get the CommandClass Instance associated with this notification.
			 * @return the instance ID
			 */
			uint8 GetInstance() const
			{
				return m_valueId.GetInstance();
			}
			...
			{
				assert((Type_NodeEvent == m_type) || (Type_ControllerCommand == m_type));
				return m_event;
			}
			...
			{
				assert(Type_CreateButton == m_type || Type_DeleteButton == m_type || Type_ButtonOn == m_type || Type_ButtonOff == m_type);
				return m_byte;
			}
			...
			{
				assert((Type_Notification == m_type) || (Type_ControllerCommand == m_type));
				return m_byte;
			}
			...
			{
				assert(Type_ControllerCommand == m_type);
				return m_command;
			}
			...
			{
				assert((Type_UserAlerts == m_type) && (Alert_ApplicationStatus_Retry == m_useralerttype));
				return m_byte;
			}
			...
			LevelChangeParameters const& GetLevelChangeParameters () const
			{
		...
		private:
			...
			void SetHomeAndNodeIds(uint32 const _homeId, uint8 const _nodeId)
			{
				m_valueId = ValueID(_homeId, _nodeId);
			}
			void SetHomeNodeIdAndInstance(uint32 const _homeId, uint8 const _nodeId, uint32 const _instance)
			{
				m_valueId = ValueID(_homeId, _nodeId, _instance);
			}
			void SetValueId(ValueID const& _valueId)
			{
				m_valueId = _valueId;
			}
			void SetGroupIdx(uint8 const _groupIdx)
			{
				assert(Type_Group == m_type);
				m_byte = _groupIdx;
			}
			void SetEvent(uint8 const _event)
			{
				assert(Type_NodeEvent == m_type || Type_ControllerCommand == m_type);
				m_event = _event;
			}
			void SetSceneId(uint8 const _sceneId)
			{
				assert(Type_SceneEvent == m_type);
				m_byte = _sceneId;
			}
			void SetButtonId(uint8 const _buttonId)
			{
				assert(Type_CreateButton == m_type || Type_DeleteButton == m_type || Type_ButtonOn == m_type || Type_ButtonOff == m_type);
				m_byte = _buttonId;
			}
			void SetNotification(uint8 const _noteId)
			{
				assert((Type_Notification == m_type) || (Type_ControllerCommand == m_type));
				m_byte = _noteId;
			}
			void SetUserAlertNotification(UserAlertNotification const alerttype)
			{
				assert(Type_UserAlerts == m_type);
				m_useralerttype = alerttype;
			}
			void SetCommand(uint8 const _command)
			{
				assert(Type_ControllerCommand == m_type);
				m_command = _command;
			}
			...
			void SetLevelChangeStartParameters( LevelChangeType type,
										LevelChangeDirection primaryDirection,
									   LevelChangeDirection secondaryDirection,
									   uint8 ignoreStartLevel,
									   uint8 primaryStartLevel,
									   uint16 durationSeconds,
									   uint8 secondaryStepSize,
									   string colorTarget )
			{
				assert(Type_LevelChangeStart == m_type);
				m_levelChangeParameters.m_type = type;
				m_levelChangeParameters.m_primaryDirection = primaryDirection;
				m_levelChangeParameters.m_secondaryDirection = secondaryDirection;
				m_levelChangeParameters.m_ignoreStartLevel = ignoreStartLevel;
				m_levelChangeParameters.m_primaryStartLevel = primaryStartLevel;
				m_levelChangeParameters.m_durationSeconds = durationSeconds;
				m_levelChangeParameters.m_secondaryStepSize = secondaryStepSize;
				m_levelChangeParameters.m_colorTarget = colorTarget;
			}
			void SetLevelChangeType ( LevelChangeType type )
			{
				assert(Type_LevelChangeStart == m_type || Type_LevelChangeStop == m_type );
				m_levelChangeParameters.m_type = type;
			}

			void SetColorTarget ( string colorTarget )
			{
				assert(Type_LevelChangeStart == m_type || Type_LevelChangeStop == m_type );
				m_levelChangeParameters.m_colorTarget = colorTarget;
			}
			...
			string m_comport;
			LevelChangeParameters m_levelChangeParameters;
	};
