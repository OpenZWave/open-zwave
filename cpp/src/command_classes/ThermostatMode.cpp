...
)
{
	if( ThermostatModeCmd_Report == (ThermostatModeCmd)_data[0] )
	{
		...
		{
			Log::Write( LogLevel_Info, "Received thermostat mode from node %d: index %d", GetNodeId(), _data[1]&0x1f );
		}
		return true;
	}
	...
	if( ThermostatModeCmd_SupportedReport == (ThermostatModeCmd)_data[0] )
	{
		...
		Log::Write( LogLevel_Info, "Received supported thermostat modes from node %d", GetNodeId() );		
		...
		for( uint32 i=1; i<_length-1; ++i )
		{
			for( int32 bit=0; bit<8; ++bit )
			{
				if( ( _data[i] & (1<<bit) ) != 0 )
				{
					...
					{
						Log::Write( LogLevel_Info, "Received unknown thermostat mode: 0x%x", item.m_value);
					}
					else
					{
						...
						Log::Write( LogLevel_Info, "    Added mode: %s", c_modeName[item.m_value] );
					}
				}
			}
		}
