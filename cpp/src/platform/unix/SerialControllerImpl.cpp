...
//-----------------------------------------------------------------------------
			SerialControllerImpl::SerialControllerImpl(SerialController* _owner) :
					m_owner(_owner), m_hSerialController(-1), m_pThread( NULL)
			{
			}
...
//-----------------------------------------------------------------------------
			void SerialControllerImpl::SerialReadThreadEntryPoint(Event* _exitEvent, void* _context)
			{
...
//-----------------------------------------------------------------------------
			void SerialControllerImpl::ReadThreadProc(Event* _exitEvent)
			{
				uint32 attempts = 0;
...
//-----------------------------------------------------------------------------
			bool SerialControllerImpl::Init(uint32 const _attempts)
			{
				...
				SerialOpenFailure: Log::Write(LogLevel_Error, "ERROR: Failed to open serial port %s", device.c_str());
				if (m_hSerialController >= 0)
				{
					close(m_hSerialController);
					m_hSerialController = -1;
...
//-----------------------------------------------------------------------------
			uint32 SerialControllerImpl::Write(uint8* _buffer, uint32 _length)
			{
				if (-1 == m_hSerialController)
				{
					//Error
