...
namespace OpenZWaveDotNet
{
	...
	/*@{*/
		 ...
		 * @return the number of switch points defined in this schedule.  Returns zero if the value is not a ZWValueID::ValueType_Schedule. The type can be tested with a call to ZWValueID::GetType.
		 */
		 ...
		 * @return true if successful.  Returns false if the value is not a ZWValueID::ValueType_Schedule. The type can be tested with a call to ZWValueID::GetType.
		 ...
		 * @param minutes The minutes part of the time when the switch point will trigger.  This value must be
		 * between 0 and 59.
		 * @return true if successful.  Returns false if the value is not a ZWValueID::ValueType_Schedule or if there 
		 * is not switch point with the specified time values. The type can be tested with a call to ZWValueID::GetType.
		 ...
		 */
		void ClearSwitchPoints( ZWValueID^ id ){ return Manager::Get()->ClearSwitchPoints(id->CreateUnmanagedValueID() ); }
		 ...
		 */
		bool GetSwitchPoint( ZWValueID^ id, uint8 idx, [Out] System::Byte %o_hours, [Out] System::Byte %o_minutes, [Out] System::SByte %o_setback );
