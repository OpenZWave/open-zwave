...
)
{
	switch( (SecurityCmd)_data[0] )
	{
		...
		{
			Log::Write( "Received SecurityCmd_SupportedReport from node %d", GetNodeId() );
			break;
		}
		case SecurityCmd_SchemeReport:
		{
			Log::Write( "Received SecurityCmd_SchemeReport from node %d", GetNodeId() );
			uint8 schemes = _data[1];
			if( schemes & SecurityScheme_Zero )
			{
				// We're good to go.  We now wait for a key to be sent by the device.
				Log::Write( "    Security scheme agreed.  Key expected from device." );
			}
			else
			{
				// No common security scheme.  The device will
				// continue as an unsecured node.
				Log::Write( "    No common security scheme.  The device will continue as an unsecured node." );
		...
		{
			Log::Write( "Received SecurityCmd_NetworkKeySet from node %d", GetNodeId() );
			break;
		}
		case SecurityCmd_NetworkKeyVerify:
		{
			Log::Write( "Received SecurityCmd_NetworkKeyVerify from node %d", GetNodeId() );
			break;
		}
		case SecurityCmd_SchemeInherit:
		{
			Log::Write( "Received SecurityCmd_SchemeInherit from node %d", GetNodeId() );
			break;
		}
		case SecurityCmd_NonceGet:
		{
			Log::Write( "Received SecurityCmd_NonceGet from node %d", GetNodeId() );
			SendNonceReport();
			break;
		}
		case SecurityCmd_NonceReport:
		{
			Log::Write( "Received SecurityCmd_NonceReport from node %d", GetNodeId() );
			EncryptMessage( &_data[1] );
			break;
		}
		case SecurityCmd_MessageEncap:
		{
			Log::Write( "Received SecurityCmd_MessageEncap from node %d", GetNodeId() );
			DecryptMessage( _data, _length );
			break;
		}
		case SecurityCmd_MessageEncapNonceGet:
		{
			Log::Write( "Received SecurityCmd_MessageEncapNonceGet from node %d", GetNodeId() );
...
)
{
	...
	AES aes();
	aes.EncryptBlock( m_initializationVector, _authentication );
	...
	for( i=0; i<numBlocks; ++i )
	{
		...
		// Encrypt the result
		aes.EncryptBlock( _authentication, _authentication );
	}
}
