...
)
{
	if( ThermostatOperatingStateCmd_Report == (ThermostatOperatingStateCmd)_data[0] )
	{
		...
		if( ValueString* valueString = static_cast<ValueString*>( GetValue( _instance, 0 ) ) )
		{
			...
			valueString->Release();
			Log::Write( LogLevel_Info, "Received thermostat operating state from node %d: %s", GetNodeId(), valueString->GetValue().c_str() );		
		}
		return true;
	}
...
)
{
	...
	{
	  	node->CreateValueString( ValueID::ValueGenre_User, GetCommandClassId(), _instance, 0, "Operating State", "", true, false, c_stateName[0] );
	}
}
