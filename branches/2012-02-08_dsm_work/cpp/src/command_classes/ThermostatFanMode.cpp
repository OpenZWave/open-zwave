		...
		for( uint32 i=1; i<_length-1; ++i )
		{
			for( int32 bit=0; bit<8; ++bit )
			{
				if( ( _data[i] & (1<<bit) ) != 0 )
				{
					...
					{
						Log::Write("Received unknown fan mode: 0x%x", item.m_value);
					}
					else
					{
...
)
{
	...
	if( Node* node = GetNodeUnsafe() )
	{
	  	node->CreateValueList( ValueID::ValueGenre_User, GetCommandClassId(), _instance, 0, "Fan Mode", "", false, false, m_supportedModes, m_supportedModes[0].m_value, 0 );
	}
}
